import { HttpException, HttpStatus, Injectable, NestMiddleware, ValidationPipe } from '@nestjs/common';
...
export class SessionAuthorizationMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if (!authToken) {
      throw new HttpException({ message: "Token not provided!" }, HttpStatus.NOT_FOUND);
    }
...
export class SessionLoginMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (!comparePassword) {
        throw new HttpException({ message: 'Incorrect email or password.' }, HttpStatus.FORBIDDEN);
      }
      req.body = user.id;
    } catch (err) {
      throw new HttpException({ message: 'Incorrect email or password.' }, HttpStatus.FORBIDDEN);
